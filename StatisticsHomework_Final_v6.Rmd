...
According to q.12, $\frac{1}{I(\theta)}=Var^*_{\theta}({\hat{\theta}}_{ML})$, which means that the Cramer Rao bound and the variance of $\hat{\theta}_{ML}$ are equal. We showed that $Var^*_{\theta}({\hat{\theta}}_{ML})=\frac{\theta}{n}$.  
Also, according to q.15a, $\sqrt{n}(\hat\theta_2 - \theta)$ converges to $\mathcal{N}(0,2\theta^2 + \theta)$ when $n\to\infty$.  
This means that $Var(\hat\theta_2) \sim \frac{2\theta^2 + \theta}{n}$ when $n\to\infty$.  
Comparing the two results, we can see that the variance of $\hat\theta_{ML}$ (or the Cramer Rao bound) converges faster to 0 when $n\to\infty$ than the variance of $\hat\theta_2$. As it is an unbiased estimator of $\theta$ with a smaller asymptotical variance, it means that it is a better estimator of $\theta$ when $n\to\infty$.
...
theme_set(theme_classic(base_size = 0.1))
...
Intuitively, we choose to compare our sample distribution to a normal law distribution of same mean and variance, for n = 43. Indeed, for each criterion, we can consider the grades attributed to the judges are i.i.d. and we can apply the CLT to them. Given the high correlations between variables, we chose not to plot them all but will study some interesting relations in the next section.  
```{r, fig.height=2}
par(mfrow=c(1,3))
