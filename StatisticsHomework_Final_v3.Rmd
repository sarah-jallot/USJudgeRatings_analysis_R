...
We have $M = \left\{p(.|\theta), \theta \in R^\star_+\right\}$ where $\forall k \in N^{\star}, p(.|\theta) = e^{-\theta}\frac{\theta^k}{k!}$ 
iii. Parameter:  
...
### Question 6.  
We know that $\hat\theta_{ML}$ is an estimator of $\theta$ (the mean as well as the variance of our Poisson law).  
...
### Question 7.  
We know that $\sqrt{n}\frac{(\hat\theta_{ML}-\theta)}{\sqrt{\hat\theta_{ML}}}$ converges in law to $N(0,1)$. 
...
Asymptotic confidence interval for $\hat\theta_2$:  
...
Thus, $\frac{\sqrt{n}(\hat\theta_2 - \theta)}{\sqrt{2\theta^2 + \theta}}$ converges to $\mathcal{N}(0,1)$ when $n\to\infty$.  
$lim_{n\to\infty}P(-q_{1-\alpha/2}\le \frac{\sqrt{n}(\hat\theta_2 - \theta)}{\sqrt{2\theta^2 + \theta}} \le q_{1-\alpha/2})=1-\alpha$ where $q_{1-\alpha/2}$ is the quantile of the standard normal distribution at level $\alpha$.  
We easily obtain: $lim_{n\to\infty}P(\theta - \frac{q_{1-\alpha/2}\sqrt{2\theta^2 + \theta}}{\sqrt{n}} \le \hat\theta_2 \le \theta + \frac{q_{1-\alpha/2}\sqrt{2\theta^2 + \theta}}{\sqrt{n}}) =1-\alpha$  
Thus, an asymptotic confidence interval for $\hat\theta_2$ at level $\alpha$ is $IC(\alpha)=[A_n,B_n]$ where:  
$A_n=\theta - \frac{q_{1-\alpha/2}\sqrt{2\theta^2 + \theta}}{\sqrt{n}}$  
$B_n=\theta + \frac{q_{1-\alpha/2}\sqrt{2\theta^2 + \theta}}{\sqrt{n}}$.  

Comment on this interval: xx  
Compare the asymptotic variance to the one of MLE and to the Cramer Rao bound. What can you say?    
...
3. PHYS is the variable that fits the least with a normal law. Given that we have a certain number of outliers and they are far from the mean, to the left, we can explain these results.  
```{r}
Corr
```
