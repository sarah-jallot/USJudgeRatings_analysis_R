...
We have $M = \left\{p(.|\theta), \theta \in R^\star_+\right\}$ where $p(.|\theta) = exp(-\theta)*\frac{\theta^k}{{k}!}$  
iii. Parameter:  
...
So, by Fisher, $\hat\theta_1$ is an efficient estimator of $\theta$: it is the estimator with minimal variance over all unbiased and linear estimators as its variance is equal to the Cramer Rao bound. 
